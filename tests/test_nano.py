...
def patch_httpx_post(fake_urlopen):
    ...
    return patch("nano.providers.clients.httpx.Client.post", fake_post)
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-Agent"] == "nano/0.1"
...
def test_openai_compatible_client_extracts_text_from_event_stream():
    ...
    with patch("nano.providers.clients.httpx.Client.post", return_value=FakeResponse()):
        result = client.complete("hello", 42)
...
def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = nano_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = nano_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_package_import_surface_includes_cli_entrypoints():
    assert callable(nano_pkg.main)
    assert callable(nano_pkg.build_agent)
    assert callable(nano_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "nano", "--help"],
        capture_output=True,
        text=True,
    )
